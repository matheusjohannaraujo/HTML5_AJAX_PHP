...
function AJAX() {
	...
	if ((typeof ajax === 'undefined' ? 'undefined' : _typeof(ajax)) == "object") {
		ajax.debug = debug;
		ajax.dump = {
			data: [],
		...
		ajax.jsonToString = function (object) {
			...
			Object.keys(ajax.params).forEach(function (key) {
				return str += key + '=' + ajax.params[key] + '&';
			});
	...
	ajax.run = function () {
		try {
			...
			if (ajax.method == 'GET' || ajax.method == 'DELETE') {
				...
				} else if (_typeof(ajax.params) == 'object' && !(ajax.params instanceof Array) && !(ajax.params instanceof FormData) && !(ajax.params instanceof Blob) && !(ajax.params instanceof Int8Array)) {
